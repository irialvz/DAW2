...
/* CREAMOS EL TABLERO */
//onload va a cargar todo el DOM del index
onload = function tablero() {
 /* REFERIMOS LA TABLA VACÍA EN UNA VARIABLE */
 varTablero = this.document.getElementById("tablero");
 ...
 for (let i=0;i<8;i++){
    ...
    for (let j=0;j<8;j++){
        ...
        switch(i){
            case 0:
            case 7:
                celda.innerHTML = `<div class=${i === 0 ? 'negras' : 'blancas'} >`+piezas[j]+"</div>";
                break;
            case 1:
            case 6:
                celda.innerHTML = `<div class=${i === 1 ? 'negras' : 'blancas'} >`+peon+"</div>";
                break;
        }
